...
## v0.31.6 — Contracts & Dispatch Coherence

- Audited the old commercial path and found one crude profile owned feedstock
  properties, downstream delivery product/target, sales price and bonus. The
  same `active_contract_id` then acted as crude provenance, delivery obligation,
  LAB volume gate, progression objective and sale terms. Buying crude therefore
  armed an output order and valid downstream/spot choices could be described as
  wrong.
- Split catalog ownership. `CrudeContractCatalog` now contains only feed grade,
  price and canonical process properties/informational expected yields.
  `ProductContractCatalog` owns finished-product contract terms and stable spot
  prices. Physical product quantity and analysis state remain tank-owned.
- Added one canonical product-contract state to `BuiltRefineryModel`: contract
  ID, delivered litres, ACTIVE/COMPLETE status and one-shot bonus flag. The
  first Area 02 contract is 200 L ON-SPEC Diesel at 8 kr/L with no bonus,
  preserving the existing target and total Standard output value.
- Made contract delivery consume and pay for exactly
  `min(available, remaining)`. Partial progress survives save/load and later
  crude batches; excess inventory remains canonical. Progression unlocks from
  the qualifying dispatched total only.
- Added explicit `LEVER TIL KONTRAKT` and `SPOTSALG` actions at Harbor PD-101.
  Spot sales preserve existing prices, never advance the active contract and
  remain valid for unrelated or excess products. Payment and optional bonus are
  emitted once by the same atomic inventory-removal transaction.
- Kept LAB-101 analytical and made first-commissioning samples possible before
  the progression unlock. LAB now reports process quality without commercial
  minimum-volume, coproduct or revenue fields. PD-101 reads the tank's canonical
  quality state. PD-201 remains a local zero-hold-up process boundary and cannot
  sell.
- Removed off-route/coproduct order gating. Heavy Residue -> VDU and VGO -> FCC
  are ordinary valid material transfers with no contract failure or progress;
  the contract only observes explicit qualifying dispatch.
- Preserved format 2 with deterministic `commercial_separation_v0316`
  migration. Incomplete commissioning receives the first Diesel contract;
  completed legacy Standard/Sour or Heavy in-flight state maps to a compatible
  one-product legacy contract. Money, pending crude, tank inventory, quality
  and progression remain unchanged, and a consumed legacy Heavy bonus cannot
  be re-armed.
- Updated CI, LAB, PD and objective text, plus model/Main/save regressions for
  crude independence, partial multi-batch delivery, overdelivery protection,
  quality blocking, spot sales, exact-once bonus, downstream processing,
  progression and migration. Process physics, world geometry and process scope
  are unchanged.

### v0.31.6 SPEC DEVIATIONS

- Format 2 is retained because the new field can be added deterministically.
- Legacy field names `active_contract_id`, tank `contract_id` and pending
  `contract_id` remain migration-compatible raw-crude provenance fields. New
  commercial truth exists only in `product_contract`; renaming every persisted
  provenance field would add risk without player value.
- Historical batch reports remain readable/valid for old saves but are no
  longer generated by Area 02 commercial dispatch. Contract delivery is a
  quantity transaction, not a whole-crude-batch closeout.
- No world/area migration was started. Human QA must approve the v0.31.6
  Pilot -> CI -> process -> LAB -> partial contract/spot -> reload loop before
  further functional relocation.
