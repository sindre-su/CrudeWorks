...
func _test_valid_topology() -> void:
	...
	_expect(network.connection_count() == 6, "complete refinery uses six directed connections")
...
func _test_cycle_rejection() -> void:
	...
	_register(network, "column", "column", "D-201")
	_register(network, "tank", "tank", "T-201")
	_expect(network.try_connect("pump", "output", "heater", "input")["ok"], "cycle fixture connects pump to heater")
	...
	_expect(network.connection_count() == 3, "cycle rejection is atomic")


func _test_removal_cleanup() -> void:
	var network = _complete_network()
	network.unregister_unit("heater")
	_expect(network.connection_count() == 4, "removing a unit removes every incident connection")
	_expect(not network.validate_configuration()["valid"], "removed process equipment invalidates the route")
...
func _test_actionable_missing_connection() -> void:
	...
	_expect("P-201" in validation["message"] and "utløp" in validation["message"], "validation identifies the first missing downstream connection")
...
func _complete_network():
	...
	network.try_connect("pump", "output", "heater", "input")
