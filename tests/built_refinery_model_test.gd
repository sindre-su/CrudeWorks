...
func _test_electrical_power_capacity() -> void:
	...
	_expect(not no_power_start["ok"] and "NO POWER" in no_power_start["message"] and not model.equipment["main_pump"]["running"], "generator OFF blocks a powered pump with an explicit start permissive")
	...
	_expect("P-201 25 kW" in model.mcc_inspection_text() and "LAB-101 5 kW" in model.mcc_inspection_text() and "LS-201 5 kW" in model.mcc_inspection_text(), "MCC inspection lists the active connected consumers centrally")
	...
	_expect("105 kW demand / 100 kW generation" in model.mcc_inspection_text(), "MCC inspection preserves the exact trip load after affected equipment stops")
	...
	model.toggle_starter_generator()
	_expect(model.reset_electrical_bus()["ok"], "restoring generation still requires and accepts an explicit MCC reset")
